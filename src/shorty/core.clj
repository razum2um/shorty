...
;; писать на Clojure, причем на достаточно *адекватном* уровне
...
;; посмотрите исходники скала-библиотек - *slick*, например.
;;
...
;; статика из ресурсов (`/...js`, `/..css`, `/index.html`),
;; потом глобальный "fallback", под который попадет только голый `/`
;; но этот роут исправится на `/index.html` через `nginx`.
;;
;; Гарантией того, что статичных роутов не может быть равно коду,
;; является алфавит без гласных.
